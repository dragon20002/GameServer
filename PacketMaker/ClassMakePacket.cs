...
namespace PacketMaker
{
    class ClassMakePacket : MakePacket
    {
        ...
        public ClassMakePacket(string filePath, string formSheet) : base(filePath, formSheet)
        {
        }
        ...
        {
            string classStartMacro = classHeader_ + nextLine_
                + scopeIn_ + nextLine_
            ...
            foreach (DataRow classRow in excelData_.Rows)
            {
                ...
                parseStr += string.Format(classStartMacro, classRow[classNameCol].ToString());
