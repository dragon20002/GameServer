...
bool IOCPServer::createListenSocket()
{
	...
	// -- SO_REUSEADDR 서버 다시 올릴 때 이전에 사용했던 소켓을 다시 OS에 요청.
	// -- 소켓이 완전히 닫힐 때까지 2~3 분 정도 걸림
...
bool IOCPServer::run() //listen, accept, worker thread 생성
{
	WSADATA wsaData;
	...
	while (!_shutdown) {
		wstring cmdLine;
		std::getline(std::wcin, cmdLine); //직접 서버에 명령어를 입력 가능
...
void IOCPServer::onAccept(SOCKET accepter, SOCKADDR_IN addrInfo) //세션 생성, 매니저에 등록
{
...
DWORD WINAPI IOCPServer::acceptThread(LPVOID serverPtr) //accept 작업, onAccept 호출
{
...
DWORD WINAPI IOCPServer::workerThread(LPVOID serverPtr)
{
	...
	while (!_shutdown) {
		...
		BOOL ret = GetQueuedCompletionStatus(
			server->iocp(), &transferSize, (PULONG_PTR)&session, (LPOVERLAPPED *)&ioData, INFINITE
		);
		if (!ret) {
			continue;
		}
